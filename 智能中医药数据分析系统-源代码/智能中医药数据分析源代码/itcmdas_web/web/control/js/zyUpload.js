	...
	$.fn.zyUpload = function(options,param){
		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					// 创建带有拖动的html
					html += '<form id="uploadForm" action="UploadFileServlet" method="post" enctype="multipart/form-data">';
	            	...
	            	html += '						<input id="fileImage" type="file" size="30" name="file" accept=".jpg, .jpeg, .png" '+multiple+'>';
	            	html += '					</div>';
	            	html += '				</div>';
		            ...
		            html += '					<div  class="upload_btn" type="submit" >开始上传</div>';
		            html += '				</div>';
		            html += '			</div>';
		            ...
		            html += '					<input class="upload_btn" type="submit" >开始上传</input>';
		            html += '				</div>';
		            html += '			</div>';
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				if (file.type.indexOf("image") == 0) {
					...
					html += '		<div class="uploadImg" style="width:'+imgWidth+'px">';				
					html += '			<img id="uploadImage_'+file.index+'" class="upload_image" src="' + e.target.result + '" style="width:250px : this.width)" />';
					html += '		</div>';
					html += '	</a>';
